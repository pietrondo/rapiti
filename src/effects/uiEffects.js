...
MenuAnimations.prototype.draw = function(ctx, palette) {
  for (var i = 0; i < this.buttons.length; i++) {
    ...
    ctx.fillStyle = btn.hover ? (palette ? palette.accent : '#d4af37') : (palette ? palette.uiBg : '#1a1a2e');
...
PuzzleAnimations.prototype.draw = function(ctx, palette) {
  for (var i = 0; i < this.pieces.length; i++) {
    ...
    if (piece.type === 'gear') {
      ...
      ctx.fillStyle = piece.solved ? (palette ? palette.accent : '#d4af37') : (palette ? palette.uiBg : '#1a1a2e');
      ctx.fillRect(-piece.w/2, -piece.h/2, piece.w, piece.h);
    }
