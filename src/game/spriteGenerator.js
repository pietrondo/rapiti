...
const SpriteGenerator = {
  ...
  _drawPlayerFrame(ctx, ox, oy, direction, frame, colors) {
    ...
    const legFrame = frame; // 0,1,2,3
    const legOffsetX = [0, 2, 0, -2][legFrame];
    const legOffsetY = [0, -1, 0, -1][legFrame];
    const bodyBob = frame % 2 === 0 ? 0 : 1;
    const armSwing = [0, 3, 0, -3][legFrame];
  ...
  _drawPiazza(ctx) {
    ...
    ctx.beginPath();
    ctx.arc(340, 25, 14, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#0a0a1a';
    ctx.beginPath();
    ctx.arc(345, 22, 11, 0, Math.PI * 2);
    ctx.fill();
