var registryData = [
  {
    year: '1952',
    detail:
      'Lena B. — 42 anni. Scomparsa il 12 luglio. Caso archiviato come "allontanamento volontario".',
  },
  {
    year: '1969',
    detail:
      'Marco F. — 28 anni. Scomparso il 3 agosto. Testimone: "una luce nel campo". Caso archiviato.',
  },
  {
    year: '1974',
    detail:
...
function setupRegistry() {
  ...
  for (var i = 0; i < slots.length; i++) {
    ...
    slot.addEventListener('drop', (e) => {
      ...
      var match = registryData.find((d) => d.year === year);
...
function updateRegistryConfirmButton() {
  var slots = document.querySelectorAll('.registry-slot');
  var allFilled = true;
  for (var i = 0; i < slots.length; i++) {
    if (!slots[i].getAttribute('data-placed-year')) {
      allFilled = false;
      break;
    }
  }
...
function checkRegistry() {
  ...
  var correct = true;
  for (var i = 0; i < slots.length; i++) {
    if (slots[i].getAttribute('data-placed-year') !== correctOrder[i]) {
      correct = false;
      break;
    }
  }
