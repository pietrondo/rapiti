...
const StoryManager = {
  ...
  completeObjective: function (objectiveId) {
    ...
    var chapter = storyChapters[this.currentChapter];
    var objective = chapter.objectives.find((obj) => obj.id === objectiveId);
  ...
  checkChapterCompletion: function () {
    ...
    var allRequiredCompleted = chapter.requiredObjectives.every(
      (reqId) => completed.indexOf(reqId) !== -1
    );
    ...
    for (var questId in this.activeQuests) {
      ...
      result.push({
        ...
        currentStageDescription: quest.stages[progress.currentStage]
          ? quest.stages[progress.currentStage].description
          : null,
      });
    }
    return result;
